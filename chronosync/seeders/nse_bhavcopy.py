"""NSE Bhavcopy seeder — downloads daily archive ZIP and upserts instruments."""
...
from chronosync.seeders._urls import nse_url
...
class NSEBhavcopySeeder:
    exchange: ClassVar[str] = "NSE"
    ...
    async def run(self, session: AsyncSession) -> int:
        ...
        url = nse_url(target)
        _log.info("nse_bhavcopy_fetch_start", url=url, on_date=target.isoformat())
        ...
        if not rows:
            raise BhavcopyFetchError(f"empty bhavcopy at {url}")
...
def _parse_zip_csv(content: bytes) -> list[dict[str, Any]]:
    with zipfile.ZipFile(io.BytesIO(content)) as zf:
        ...
        if csv_name is None:
            raise BhavcopyFetchError("no CSV in NSE bhavcopy ZIP")
        with zf.open(csv_name) as fp:
            text = io.TextIOWrapper(fp, encoding="utf-8")
            reader = csv.DictReader(text)
            return [
                _row_to_instrument(r)
                for r in reader
                if _row_to_instrument(r) is not None  # type: ignore[misc]
            ]


def _row_to_instrument(r: dict[str, str]) -> dict[str, Any] | None:
    # Modern NSE Bhavcopy header names (BhavCopy_NSE_CM_0_0_0_{date}_F_0000.csv).
    # Legacy header names also supported as a fallback.
    series = (r.get("SctySrs") or r.get("SERIES") or "").strip().upper()
    if series not in ("EQ", "BE", "BZ", "SM", "ST"):
        return None
    ticker = (r.get("TckrSymb") or r.get("SYMBOL") or "").strip().upper()
    if not ticker:
        return None
