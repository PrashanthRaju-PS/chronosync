...
async def test_full_iteration_and_idempotency(db_session, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    df = pd.DataFrame(
        [
            {"Open": 100.0, "High": 105.0, "Low": 99.0, "Close": 104.0, "Adj Close": 104.0, "Volume": 1000},
            {"Open": 104.0, "High": 110.0, "Low": 103.0, "Close": 108.0, "Adj Close": 108.0, "Volume": 2000},
            {"Open": 108.0, "High": 112.0, "Low": 107.0, "Close": 111.0, "Adj Close": 111.0, "Volume": 1500},
        ],
    ...
    tasks = planner.split_backfill(
        ...
        date(2024, 1, 2), date(2024, 1, 4), batch_days=10,
    )
    ...
    bars = await repos.daily_bars_range(db_session, instrument_id=inst.id, frm=date(2024, 1, 1), to=date(2024, 1, 10))
    assert len(bars) == 3
    ...
    bars2 = await repos.daily_bars_range(db_session, instrument_id=inst.id, frm=date(2024, 1, 1), to=date(2024, 1, 10))
